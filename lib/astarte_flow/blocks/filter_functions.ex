...
defmodule Astarte.Flow.Blocks.FilterFunctions do
  ...
  def make_filter(%{operator: :==, value: fvalue}) do
    fun = fn
      ...
      any_invalid ->
        _ = Logger.warning("FilterFunctions: invalid data: #{inspect(any_invalid)}")
        false
    end
  ...
  def make_filter(%{operator: :luerl_script, script: lua_script}) do
    ...
    case :luerl.load(lua_script, luerl_state) do
        ...
        fun = fn
          %Message{} = msg ->
            ...
            case :luerl.eval(chunk, state) do
              ...
              any_unexpected ->
                _ =
                  Logger.warning("FilterFunctions: luerl unexpected: #{inspect(any_unexpected)}")
      ...
      {:error, reason, _} ->
        _ = Logger.warning("FilterFunctions: luerl error: #{inspect(reason)}")
