...
defmodule Astarte.FlowWeb.Plug.VerifyHeader do
  @moduledoc """
  ...
  defp get_secret(conn) do
    with {:ok, realm} <- Map.fetch(conn.path_params, "realm"),
         {:ok, public_key_pem} <- Auth.fetch_public_key(realm),
         %JWK{} = jwk <- JWK.from_pem(public_key_pem) do
      jwk
    else
      [] ->
        # This is returned by JWK.from_pem if it fails to parse the key
        nil

      :error ->
        # No realm in path params
        nil
